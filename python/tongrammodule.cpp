...
static PyObject* tongram_load_by_type(PyObject* self, PyObject* args,
                                      std::string model_string_type) {
    const char* binary_filename;
    if (!PyArg_ParseTuple(args, "s", &binary_filename))
        return NULL;
    ...
    std::cout << "\tBytes per gram: " << double(file_size) / model_p->size()
              << std::endl;

    return Py_BuildValue(
        "ks", model_p,
        model_string_type.c_str());  // unsigned long.
...
#undef LOOP_BODY
    ...
    oss << "Error: tongram_load() not supported with type "
        << "'" << model_string_type << "' [" << binary_filename << "].";
...
template <typename Model>
static PyObject* tongram_lookup_by_type(PyObject* self,
...
static PyObject* tongram_lookup(PyObject* self, PyObject* args) {
    ...
    if (!PyArg_ParseTuple(args, "(ls)s", &pointer_as_long, &model_string_type,
                          &ngrams_space_separated))
        return NULL;  // https://docs.python.org/3/extending/extending.html
...
#define LOOP_BODY(R, DATA, T)                                   \
        ...
        return tongram_lookup_by_type<T>(self, pointer_as_long, \
                                         ngrams_space_separated);
...
#undef LOOP_BODY
    ...
    std::ostringstream oss;
    oss << "Error: tongram_lookup() not supported with type "
        << "'" << model_string_type;
...
template <typename Model>
static PyObject* tongram_score_by_type(PyObject* self,
                                       unsigned long pointer_as_long,
                                       const char* ngrams_space_separated) {
    Model* pointer_to_model = (Model*)pointer_as_long;
    ...
    text_lines corpus(ngrams_space_separated);
    state.init();

    while (!corpus.end_of_line()) {
        corpus.next_word(word);
        pointer_to_model->score(state, word, is_OOV, log10_prob);
        //        std::cout << "\"Word\" : \"" << std::string(word.first,
        //        word.second) << "\", " << "\"log10_prob\" : " << log10_prob <<
        //        "\n";
...
#define LOOP_BODY(R, DATA, T)                                  \
        ...
        return tongram_score_by_type<T>(self, pointer_as_long, \
                                        ngrams_space_separated);
...
#undef LOOP_BODY
    ...
    std::ostringstream oss;
    oss << "Error: tongram_score() not supported with type "
        << "'" << model_string_type;
...
static PyMethodDef TongramMethods[] = {
    ...
    {"lookup", tongram_lookup, METH_VARARGS,
     "Look up a space separated ngram."},
