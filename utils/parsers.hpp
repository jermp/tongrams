...
namespace tongrams
{
    ...
    prob_backoff_record parse_prob_backoff_line(std::string const& line)
    {
        ...
        if (pos != br.second) {
            record.backoff = std::strtod(reinterpret_cast<const char*>(pos), nullptr);
        }
    ...
    struct prob_backoff_line_handler
    {
        ...
        {
            formatted_line = std::to_string(record.prob)
                           + "\t" + std::string(record.gram.first,
                                                record.gram.second)
                           // omit 0.0 backoffs
    ...
    struct arpa_parser
    {
        ...
        arpa_parser(char const* arpa_filename)
            : m_is(std::ifstream(arpa_filename))
            , m_cur_line_num(0)
        {
            if (!m_is.good()) {
                throw std::runtime_error("error in opening arpa file, it may not exist.");
            }
        }
        ...
        bool read_line() {
            ...
            if (m_cur_line_num % 100000000 == 0) {
                util::logger("Processed " + std::to_string(m_cur_line_num) + " lines");
            }
    ...
    {
        typedef count_records_iterator<boost::iostreams::filtering_istream> iterator;

        grams_gzparser(char const* grams_filename)
            : m_is(std::ifstream(grams_filename, std::ios_base::in | std::ios_base::binary))
            , m_num_lines(0)
        {
            if (!m_is.good()) {
                throw std::runtime_error("error in opening grams file, it may not exist.");
            }
            ...
            if (line.empty()) {
                throw std::runtime_error("first line must be non-empty and contain the number of lines.");
            }
            ...
            try {
                ...
                std::cerr << "firt line must contain the number of lines." << std::endl;
                exit(1);
            }
        }
