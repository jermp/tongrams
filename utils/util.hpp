...
namespace tongrams
{
    ...
    namespace bytes
    {
        ...
        // is always present
        byte_range split_upon(std::string const& s,
        ...
        byte_range split_upon_check_end(std::string const& s,
                                        const char separator)
        {
            const uint8_t* begin =
                reinterpret_cast<uint8_t const*>(s.c_str());
    ...
    namespace building_util
    {
        ...
        void check_remapping_order(uint8_t order) {
            if (order > global::max_remapping_order) {
                throw std::invalid_argument("tongrams max supported remapping order is "
        ...
        void display_legend() {
            std::cout << "Legend:\n"
                      << "  " << style::bold << "bold     " << style::off << "  means mandatory argument\n"
                      << "  " << style::underline << "underline" << style::off << "  means replaceable argument\n"
        ...
        void print_general_params() {
            std::cout
                << "\t[--u " << style::underline << "value" << style::off << "]\n"
                << "\t[--p " << style::underline << "value" << style::off << "]\n"
                << "\t[--b " << style::underline << "value" << style::off << "]\n"
                << "\t[--arpa " << style::underline << "arpa_filename" << style::off << "]\n"
                << "\t[--out " << style::underline << "output_filename" << style::off << "]"
                << std::endl;
        }
        ...
        void print_general_info() {
            std::cout
                << "-------------------------------------------------------------------------------------------------------\n"
                << style::bold << style::underline << "order" << style::off << " defines the maximum-order of the grams.\n"
                << style::bold << style::underline << "value_type" << style::off << " is either 'count' or 'prob'.\n"
                << "'--u' specifies the log10 probability of the unknown word.\n"
                << "'--p' specifies the number of quantization bits for probability values (default is 8).\n"
                << "'--b' specifies the number of quantization bits for backoff values (default is 8).\n"
                << "'--arpa' specifies the input arpa file.\n"
                << "'--out' specifies the output binary filename. If omitted it will be given the name of the model type.\n"
                << "If 'count' is specified as " << style::bold << style::underline << "value_type" << style::off << ", then '--arpa' option will be ignored.\n"
                << "If '--p' and '--b' are only valid with 'prob' " << style::bold << style::underline << "value_type" << style::off << " specified."
                << std::endl;
        }
            ...
            if (!boost::filesystem::exists(dir)) {
                ...
                if (begin != end) {
                    std::cerr << "Warning: " << tmp_dir
                              << " already exists and it is not empty."
                              << std::endl;
                    std::abort();
                }
            }
    ...
    namespace util
    {
        ...
        template<typename T>
        void save(uint8_t header, T const& data_structure, char const* output_filename) {
        ...
        size_t load(T& data_structure, char const* binary_filename) {
            ...
            if (!is.good()) {
                throw std::runtime_error("Error in opening binary file, it may not exist or be malformed.");
            }
            uint8_t header = 0;
            load_pod(is, &header);
