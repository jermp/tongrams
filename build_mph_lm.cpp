...
int main(int argc, char** argv)
{
    ...
    if (argc < 5 || building_util::request_help(argc, argv)) {
        ...
        std::cout << "Usage " << argv[0] << ":\n"
                  ...
                  << "\t" << style::bold << style::underline << "hash_key_bytes" << style::off << "\n"
                  << "\t" << style::bold << style::underline << "count_bytes" << style::off << "\n"
        ...
        std::cout << style::underline << "input_dir" << style::off << " is the directory containing the N-gram counts files. "
                  ...
                  << std::endl;
        std::cout << style::bold << style::underline << "hash_key_bytes" << style::off
                  << " specifies the number of bytes used for the hash keys: it must be 4 or 8."
                  << std::endl;
        std::cout << style::bold << style::underline << "count_bytes" << style::off
                  << " specifies the number of bytes used for the distinct counts: it must be 4 or 8."
                  << std::endl;
        return 1;
    }
    ...
    for (int i = 4; i < argc; ++i) {
        if (argv[i] == std::string("count")) {
            ...
            std::cout << "substituting <unk> probability with: "
                      << unk_prob << std::endl;
