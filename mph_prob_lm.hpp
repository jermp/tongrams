        ...
        struct builder
        {
                ...
                , m_unk_prob(unk_prob)
            {
                ...
                for (uint8_t order = 1; order <= m_order; ++order)
                {
                    ...
                    if (order != 1) { // need scan uni-grams anyway to set arpa offsets
                ...
                for (uint8_t order = 2; order <= m_order; ++order)
                {
                    ...
                    arpa_iterator it(m_arpa_filename, order, arpa_offsets[order - 1]);
                    ...
                    for (auto const& record: pool_index) {
                        ...
                        uint64_t prob_rank =
                            probs_builder.rank(order - 2, prob, 0);
