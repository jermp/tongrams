...
struct compact_vector {
    ...
    private:
        ...
        void read() {
            if (m_cur_shift + m_data->m_width <= 64) {
                m_cur_val =
                    m_data->m_bits[m_cur_block] >> m_cur_shift & m_data->m_mask;
            } else {
                uint64_t res_shift = 64 - m_cur_shift;
                m_cur_val = (m_data->m_bits[m_cur_block] >> m_cur_shift) |
                            (m_data->m_bits[m_cur_block + 1] << res_shift &
                             m_data->m_mask);
                ++m_cur_block;
    ...
    struct builder {
        builder(uint64_t n = 0, uint64_t w = 0)
            ...
            if (m_width > 64) {
                std::cerr << "Error: width must be <= 64." << std::endl;
                std::terminate();
            }
        }
    ...
    inline uint64_t access(uint64_t i) const {
        ...
        return (*(reinterpret_cast<uint64_t const*>(ptr + (pos >> 3))) >>
                (pos & 7)) &
               m_mask;
    }
    ...
    size_t bytes() const {
        return m_bits.size() * 8 + sizeof(m_size) + sizeof(m_width) +
               sizeof(m_mask);
    }
