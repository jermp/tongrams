...
void perf_test(const char* query_filename,
               const char* binary_filename, uint32_t runs)
{
    strings_pool sp;
    ...
    if (false) {
        ...
        std::cout << "\tMean per run: " << avg / 1000000 << " [sec]" << std::endl;
        std::cout << "\tMean per query: " << avg / test_strings << " [musec]" << std::endl;
    }
}
...
int main(int argc, char** argv)
{
    if (argc < 4 || building_util::request_help(argc, argv)) {
        ...
        std::cout << "Usage " << argv[0] << ":\n"
                  ...
                  << "\t" << style::bold << style::underline << "query_filename" << style::off << "\n"
                  << "\t" << style::bold << style::underline << "runs" << style::off
                  << std::endl;
        return 1;
    }
