...
void score_corpus(const char* binary_filename,
                  const char* corpus_filename)
{
    ...
    while (!corpus.end_of_file()) { // assume one sentence per line
        ...
        while (!corpus.end_of_line()) {
            ...
            // std::cerr << "\"word\" : \"" << std::string(word.first, word.second) << "\", "
    ...
    std::cout << "tot_log10_prob_only_OOVs = " << tot_log10_prob_only_OOVs << std::endl;
    std::cout << "perplexity including OOVs = "
              << pow(10.0, -(tot_log10_prob / static_cast<double>(corpus_tokens)))
              << std::endl;
    if (corpus_tokens - tot_OOVs) {
        std::cout << "perplexity excluding OOVs = "
                  << pow(10.0, -((tot_log10_prob - tot_log10_prob_only_OOVs)
...
int main(int argc, char** argv)
{
    if (argc < 3 || building_util::request_help(argc, argv)) {
        ...
        std::cout << "Usage " << argv[0] << ":\n"
                  ...
                  << "\t" << style::bold << style::underline << "corpus_filename" << style::off
                  << std::endl;
        return 1;
    }
