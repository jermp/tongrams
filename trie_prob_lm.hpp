...
namespace tongrams
{
    ...
    struct trie_prob_lm
    {
        ...
        struct builder
        {
            ...
            builder(const char* arpa_filename,
                    uint8_t order,
                ...
                , m_unk_prob(unk_prob)
            {
                ...
                if (m_order > counts.size()) {
                    throw std::invalid_argument("specified order exceeds arpa file order");
                }
                ...
                for (uint8_t order = 1; order <= m_order; ++order)
                {
                    ...
                    if (order != 1) { // unigrams are NOT quantized
                        ...
                        if (order != m_order) {
                            backoffs_builder.build_backoffs_sequence(backoffs, backoffs_quantization_bits);
                        }
                    }
                }
                ...
                for (uint8_t order = 2; order <= m_order; ++order)
                {
                    ...
                    sa_builder.build(m_arrays[order - 1], pointers, order, value_type::prob_backoff);
            ...
            void build(trie_prob_lm<Vocabulary,
                                    Mapper,
                                    Values,
                                    Ranks,
                                    Grams,
        ...
        private:
            ...
            {
                arpa_iterator it(m_arpa_filename, 1,
                                 unigrams_arpa_offset);
                ...
                m_vocab.build(bytes,
                              ...
                              compact_vector(values_cvb),
                              identity_adaptor());
            }

            template<typename T>
            void build_ngrams(uint8_t order,
                              uint64_t curr_order_arpa_offset,
                              ...
                              typename sorted_array_type::builder& sa_builder)
            {
                ...
                arpa_iterator curr_it(m_arpa_filename, order, curr_order_arpa_offset);
                arpa_iterator prev_it(m_arpa_filename, order - 1, prev_order_arpa_offset);
                uint64_t curr_n = curr_it.num_grams();
                uint64_t prev_n = prev_it.num_grams();
                auto prev_record = prev_it.next();
                uint8_t probs_quantization_bits = probs_builder.quantization_bits(order - 2);
                ...
                for (uint64_t i = 0; i < curr_n; ++i)
                {
                    ...
                    if (Mapper::context_remapping && order > m_remapping_order + 1) {
                        token_id.first = m_mapper.map_id(gram, token_id.first, &m_vocab,
                                                         &m_arrays.front(),
                                                         m_remapping_order,
                                                         true); // BACKWARD trie
                    }
                    ...
                    uint64_t prob_rank = probs_builder.rank(order - 2, record.prob, 0);
                    uint64_t rank = prob_rank;
                    if (order != m_order) {
                        uint64_t backoff_rank = backoffs_builder.rank(order - 2, record.backoff, 1);
            ...
            if (word_id.first != global::not_found)
            {
                ...
                for (; order_m1 <= state.length; ++order_m1, ++words_rbegin)
                {
                    ...
                    if (Mapper::context_remapping && order_m1 > m_remapping_order) {
                        id = m_mapper.map_id(prev_id, prev_prev_id, // pass the two parent ids for remapping
                                             id,
                                             &m_arrays.front(),
                                             m_remapping_order);
                    }
                    ...
                    uint64_t backoff_rank = prob_backoff_rank >> probs_quantization_bits;
