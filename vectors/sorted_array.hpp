...
namespace tongrams
{
    ...
    struct sorted_array
    {
        struct builder
        {
            ...
            builder(uint64_t num_grams,
                    uint64_t max_gram_id,
                    uint64_t max_count_rank,
            ...
            template<typename T>
            void build(sorted_array& sa, T& pointers,
                       uint8_t order, int value_t)
            {
                compact_vector grams(m_grams);
                sa.m_grams.build(grams.begin(),
                                 grams.size(),
                                 pointers, order);
            ...
            void build_counts_ranks(sorted_array& sa, uint8_t order) {
                ...
                sa.m_counts_ranks.build(counts_ranks.begin(),
                                        counts_ranks.size(), order);
            ...
            void build_pointers(sorted_array& sa, T& pointers) {
        ...
        void save(std::ostream& os,
                  uint8_t order, int value_t) const
        {
