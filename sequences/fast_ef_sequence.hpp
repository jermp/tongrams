...
namespace tongrams
{
    struct fast_ef_sequence
    {
        ...
        void find(pointer_range const r, uint64_t id, uint64_t* pos)
        {
            ...
            //     // an id that could NOT be present, as it is for the perplexity benchmark
            //     // assert(id >= operator[](lo)
            //     //     && id <= operator[](hi - 1) // NOTE: not id < operator[](hi)
            //     //                                 // because the sequence could NOT be strictly increasing
            //     //                                 // and it could be that operator[](hi) == operator[](hi - 1)
            //     //     );
            // }
            ...
            if (run >= sampling_threshold)
            {
                ...
                uint64_t tree_height = util::ceil_log2(run)
                                     - log2_sampling_threshold;
        ...
        struct iterator
        {
            iterator(bit_vector const& high_bits,
                     bit_vector const& low_bits,
                     darray1 const& high_bits_d1,
                     uint8_t l,
                     uint64_t i = 0)
    ...
    private:
        ...
        void scan(uint64_t lo, uint64_t hi,
                  uint64_t id, uint64_t* pos) const
        {
            iterator it(m_high_bits, m_low_bits,
                        m_high_bits_d1, m_l, lo);
            size_t run = hi - lo;
                  ...
                  uint64_t lower_bound) const
        {
            ...
            it.skip0(1);
            uint64_t run = std::min(it.position() - v // # of integers sharing the same high part
        ...
        void bsearch_scan(uint64_t lo, uint64_t hi,
                          uint64_t id, uint64_t* pos,
                          uint64_t lower_bound) const
        {
        ...
        static void fill_samplings(uint64_t lo, uint64_t hi,
        ...
        template<typename RandomAccessIterator>
        void build(RandomAccessIterator begin,
        ...
        {
            {
                ...
                for (uint64_t i = 1; i < pointers.size(); ++i) {
                    ...
                    if (range >= sampling_threshold) {
                        ...
                        uint64_t tree_height = util::ceil_log2(range)
                                             - log2_sampling_threshold;
                        // push previous range upperbound
                        samplings.push_back(ptr_begin ? begin[ptr_begin - 1] : 0);
                        fill_samplings(ptr_begin, ptr_end,
                                       tree_height, begin, samplings);
                    }
                    ptr_begin = ptr_end;
                }
