    ...
    struct quantized_sequence_collection
    {
        struct builder {
            ...
            {    
                ...
                auto above = std::lower_bound(data.begin() + reserved, data.end(), value);
                ...
                --prev_above;
                auto to_ret = above - data.begin() - (value - *prev_above < *above - value);
                           ...
                           size_t num_bins)
            {
                ...
                for (uint32_t i = 0; i < num_bins; ++i, start = finish) {
                    finish = sequence.begin() + ((sequence.size() * static_cast<uint64_t>(i + 1)) / num_bins);
                    if (finish == start) {
                        averages.push_back(i ? averages.back() : -std::numeric_limits<float>::infinity());
                    } else {
                        averages.push_back(std::accumulate(start, finish, 0.0) / static_cast<float>(finish - start));
                    }
                }
            }
            ...
            void check_quantization_bits(uint8_t quantization_bits, size_t n) {
                if (quantization_bits < 2 || quantization_bits > 32) {
                    throw std::invalid_argument("quantization bits must be in [2, 32]");
                }
                uint64_t num_bins = uint64_t(1) << quantization_bits;
                if (n && n < num_bins) {
                    std::cerr << "too many quantization bits used: these must be in [2, "
                              << util::ceil_log2(n) << "]"
                              << std::endl;
                    std::abort();
                }
        ...
        {
            os.write(reinterpret_cast<char const*>(m_quantization_bits.data()),
    ...
    struct byte_aligned_sequence_collection
    {
        struct builder
        {
            ...
            uint64_t rank(uint8_t order_m1, uint64_t value) const {
                ...
                uint64_t rank = 0;
                if (!util::binary_search(distinct_values,
